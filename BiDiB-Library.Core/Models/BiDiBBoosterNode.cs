    ...
    public sealed class BiDiBBoosterNode : ModelBase, IDisposable
    {
        ...
        public void TurnOn(bool withDcc, bool withWatchdog, bool broadCast)
        {
            if (withDcc)
            {
                ...
                CommandStationStateMessage stateMessage = Node.MessageProcessor.SendMessage<CommandStationStateMessage>(new CommandStationSetStateMessage(state, Node.Address));
            ...
            Node.MessageProcessor.SendMessage(Node, BiDiBMessage.MSG_BOOST_ON, (byte)(broadCast ? 0x00 : 0x01));
        }
        ...
        public void TurnOff(bool withDcc, bool broadCast)
        {
            if (withDcc)
            {
                Node.MessageProcessor.SendMessage(new CommandStationSetStateMessage(CommandStationState.BIDIB_CS_STATE_OFF, Node.Address));
                StopWatchdog();
            }

            Node.MessageProcessor.SendMessage(Node, BiDiBMessage.MSG_BOOST_OFF, (byte)(broadCast ? 0x00 : 0x01));
        ...
        private void HandleWatchdogTimerOnElapsed(object sender, ElapsedEventArgs elapsedEventArgs)
        {
            ...
            logger.LogDebug($"Watchdog event {Node.GetFullAddressString()} i:{watchdogTimer.Interval} w:{watchdogWatch.ElapsedMilliseconds} d:{duration} dcc:{DccState}");
